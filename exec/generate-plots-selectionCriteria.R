...
sc_data_paper <- 
  readRDS("./results/Results_Selection_Criteria_block_high_adjacent.RDS")

ss_data_paper <-
  readRDS("./results/Results_Stability_Selection_block_high_adjacent.RDS")
...
criterion_mean_best_results <- 
  results_paper %>% 
  filter(rho == 0.7 & 
         method %in% c("Enet 0.1", "Enet 0.5", "Enet 0.9", "Lasso", "FSS", "BSS")) %>%
  group_by(method, snr, criterion) %>%
  summarise(meanF1 = mean(F1)) %>%
  group_by(method, snr) %>%
  mutate(max_meanF1 = max(meanF1)) %>% 
  filter(meanF1 == max_meanF1) %>%
  distinct(method, snr, .keep_all= TRUE)
...
# rename 
names(best_criterion_results)[c(1,10, 11)] <- 
  c("Method", 
    "Precision",
    "Recall")
...
ggplot(best_criterion_results ,
  ...
  scale_fill_manual(values=c(
    colorRampPalette(c("#FF99CC", "#B266FF"))(3), #Enet colors
    "#FF3333", # Lasso color
    "#0080FF", # FSS color
    "#00CC00" # BSS color
  )) + 
